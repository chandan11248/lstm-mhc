"""Model B: Naive HC-LSTM (unconstrained Hyper-Connections).

Ablation model — uses multi-stream residual connections with UNCONSTRAINED
learnable matrices. No Sinkhorn-Knopp projection, no manifold constraint.

Expected behaviour: gradient instability (explosion/vanishing) as depth
increases, replicating the findings of the DeepSeek mHC paper.
"""
...
class HCLSTMBlock(nn.Module):
    """One inter-layer HC block WITHOUT manifold constraints.

    Flow::

        X_l (B, T, n, d)
        → RMSNorm + Flatten → X_norm (B, T, nd)
        → Compute H_pre, H_post, H_res (UNCONSTRAINED)
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.heads = MuHCHeads.from_config(config, constrained=False)
...
class HCLSTM(nn.Module):
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.blocks = nn.ModuleList([HCLSTMBlock(config) for _ in range(self.num_layers)])
